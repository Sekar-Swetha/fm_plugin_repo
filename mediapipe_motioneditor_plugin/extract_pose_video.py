"""MediaPipe drop-in for MotionEditor's `data_preparation/video_skeletons.py`.

Same CLI surface, same input/output layout, same filename conventions.

Usage:
    python extract_pose_video.py -d /path/to/data/case-1/images -c openposefull
...
Also accepts a video file (`-d /path/to/video.mp4`) and `--write-keypoints`
to dump per-frame JSON.
"""
...
def main():
    ...
    parser.add_argument("-d", "--data", type=str, required=True,
                        help="Folder of frames or a video file path.")
    parser.add_argument("-c", "--which_cond", type=str, default="openposefull",
                        help="Output subfolder name.")
    parser.add_argument("--min-conf", type=float, default=0.3,
                        help="Per-landmark visibility threshold.")
    parser.add_argument("--model-complexity", type=int, default=2, choices=[0, 1, 2],
                        help="MediaPipe Pose model complexity (2 = best).")
    parser.add_argument("--write-keypoints", action="store_true",
                        help="Also dump keypoints.json next to PNGs.")
    ...
    if detect_rate < 0.95:
        print(
            "WARNING: detection rate below 95% — review output PNGs and "
